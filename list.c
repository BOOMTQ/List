...
list *newList(item e){ // create an empty list and return a pointer to it
   list *l = malloc (sizeof(list));
   node *sen = malloc (sizeof(node)); // *sen = pointer to the sentinel node
   node *a;
   node *b;
   a = sen;
   b = sen;
   sen -> back = sen;
   sen -> next = sen;
   sen -> x = e; // item x is the default item e initially
    *l = (list) {a, b};
   return l;
}
...
item get(list *xs){
  item x = xs -> current -> x; // Get the current item.
  if (xs -> current == xs -> none) return -1; // none means no item selected
  else
  return x;
...
bool set(list *xs, item x){
  if (xs -> current == xs -> none) return false;
  if ((xs -> none -> next == xs -> none) && (xs -> none -> back == xs -> none))
  return false;
  else
    xs -> current -> x = x;
  return true; // Set the current item and return true.
}
...
// a new 'middle' node being inserted between (up to) two existing ones
void insertAfter(list *xs, item x){
  node *middle = malloc (sizeof(node)); // memory allocation by malloc() function
  node *q;
  node *r;
  middle -> x = x;
  q = xs -> current;
